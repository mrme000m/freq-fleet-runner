#!/usr/bin/env python3
"""grid.agents.llm — LLM provider chain (CF primary, NVIDIA / OpenRouter / Mistral fallback).

Ported from grid-autonomy/llm/provider.py (unchanged behavior; this repo
hosts the FT-fleet side of the same autonomy loop, so the chain surface
is identical — `chat`, `chat_json`, `named_chain`, `role_chain`, `ping`).

Pluggable, stdlib-only (urllib). Provider order and models come from env
so the harness stays extendable without code edits.
...
Env:
  ...
  GRID_LLM_ROLES (JSON object mapping role → provider, see ROLE_KEYS)

Usage:
  python3 -m grid.agents.provider --ping [--json]
  python3 -m grid.agents.provider --chat "summarize: ..."
  from grid.agents import chat, chat_json
...
# "optimizer" is the fast-cycle slot arbiter (swarm_arbiter) — pinned to a
# dedicated provider so the tactical layer runs on a different model than
# the debate chain.
...
def _openai_compat_chat(base_url, key, model, messages, max_tokens=1024,
                        extra_headers=None):
    if not key:
...
def _has_creds(name):
    ...
    Cred-less providers are filtered OUT of the chain at boot, not at call
    time — `_providers()` filters them, so the chain only contains
    callable entries."""
    if name == "cf":
...
def _providers():
    ...
    fns = {
        ...
        "openrouter": lambda msgs, mt: _openai_compat_chat(
            ...
            os.environ.get("OPENROUTER_MODEL", OPENROUTER_MODEL_DEFAULT),
            msgs, mt,
            {"HTTP-Referer": "https://github.com/mrme000m/tvcli",
             "X-Title": "tvcli-grid-autonomy"}),
...
def role_chain(role):
    """Single-provider chain for a role, or None when unassigned.

    GRID_LLM_ROLES is a JSON object mapping a role key (see ROLE_KEYS) to
    a provider name (cf|nvidia|openrouter|mistral). Returns [(name, fn)]
    for that one provider; returns None when the role is absent or its
    provider has no credentials (caller degrades to the global chain)."""
    if not role:
        return None
    try:
        ...
        return None
    provider_name = roles.get(role) if isinstance(roles, dict) else None
    if not provider_name:
        return None
    return named_chain(provider_name)


def named_chain(name):
    """Single-provider chain [(name, fn)] when `name` has credentials, else [].

    Public pinning primitive: callers that want one specific provider
    (e.g. the swarm arbiter on mistral) pass the result as
    chat_json(_chain=...) and fall back to the global chain on []."""
    if not name:
        return []
...
def chat(messages, max_tokens=1024, _chain=None, role=None):
    ...
    `role` (optional) pins the call to a single provider via
    GRID_LLM_ROLES; ignored when the role is unassigned or its provider
    lacks creds."""
...
def chat_json(messages, max_tokens=4096, _chain=None, attempts=3, role=None):
    ...
    the chain up to `attempts` times with a fresh completion and
...
def ping():
    """Probe every cred-bearing provider in chain order. Never raises."""
    results = []
...
def main():
    ...
    for r in results:
        ...
        print(f"{r['provider']}: {status} ({r['latency_ms']}ms)",
              file=sys.stderr)
